                ...
                {module.indicators.map((indicator) => (
                  <div key={indicator} className="rounded-[20px] border border-white/10 bg-black/10 px-4 py-3">
                    ...
                    <p className="mt-1 text-xs leading-6 text-white/58">
                      Interfaz preparada para seguimiento, operacion y trazabilidad.
                    </p>
                  </div>
                ))}
              </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.55fr_1fr]">
        <div className={`${SURFACE} p-6 md:p-7`}>
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--condome-orange)]">Flujo de trabajo</p>
              <h2 className="mt-2 text-xl font-semibold text-[var(--fg-primary)]">Lo que debe poder hacer el equipo gestor</h2>
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="mt-4 space-y-3">
              {module.related.map((item) => (
                <Link
                  ...
                  className="block rounded-[22px] border border-[var(--border-standard)] bg-[var(--surface-2)] px-4 py-3 text-inherit no-underline transition-all hover:border-[var(--control-border-strong)] hover:shadow-[var(--shadow-whisper)]"
                >
                  ...
                  <p className="mt-1 text-xs leading-6 text-[var(--fg-secondary)]">
                    Navega entre modulos conectados al mismo flujo operativo.
                  </p>
                </Link>
              ))}
            </div>
          ...
          <div className={`${SURFACE} p-6`}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--fg-tertiary)]">Vista recomendada</p>
            <div className="mt-4 rounded-[24px] border border-dashed border-[var(--border-emphasis)] bg-[var(--surface-3)] p-5">
              <div className="grid gap-3">
                {[
                  ["Resumen superior", "KPIs, filtros y acciones principales del modulo."],
                  ["Listado o agenda", "Tabla, tarjetas o seguimiento segun el caso."],
                  ["Bitacora viva", "Cambios recientes y acciones relacionadas sin perder contexto."],
                ].map(([title, copy]) => (
                  <div key={title} className="rounded-[20px] border border-[var(--border-standard)] bg-[var(--surface-2)] p-4">
