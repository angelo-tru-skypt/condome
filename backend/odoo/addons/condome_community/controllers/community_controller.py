...
class CondomeCommunityController(http.Controller):
    ...
    @http.route("/condome_api/resident/incidencias/", type="http", auth="public", methods=["GET", "POST", "OPTIONS"], csrf=False)
    def resident_incidents(self, **kwargs):
        return resident_incident_service.handle_incidents()

    @http.route("/condome_api/owner/incidencias/", type="http", auth="public", methods=["GET", "OPTIONS"], csrf=False)
    def owner_incidents(self, **kwargs):
        return owner_incident_service.handle_incidents()

    @http.route("/condome_api/owner/incidencias/<int:incidencia_id>", type="http", auth="public", methods=["PUT", "OPTIONS"], csrf=False)
    def owner_incident_update(self, incidencia_id, **kwargs):
        return owner_incident_service.handle_incident_update(incidencia_id)

    @http.route("/condome_api/owner/documentos/", type="http", auth="public", methods=["GET", "POST", "OPTIONS"], csrf=False)
    def documents(self, **kwargs):
        return community_management_service.handle_documents()

    @http.route("/condome_api/owner/documentos/<int:documento_id>", type="http", auth="public", methods=["PUT", "OPTIONS"], csrf=False)
    def document_update(self, documento_id, **kwargs):
        return community_management_service.handle_document_update(documento_id)

    @http.route("/condome_api/owner/configuracion/", type="http", auth="public", methods=["GET", "PUT", "OPTIONS"], csrf=False)
    def settings(self, **kwargs):
        return community_management_service.handle_settings()

    @http.route("/condome_api/owner/notificaciones/reglas/", type="http", auth="public", methods=["GET", "POST", "OPTIONS"], csrf=False)
    def notification_rules(self, **kwargs):
        return community_management_service.handle_notification_rules()

    @http.route("/condome_api/owner/notificaciones/reglas/<int:rule_id>", type="http", auth="public", methods=["PUT", "OPTIONS"], csrf=False)
    ...
    @http.route("/condome_api/owner/comunicados/<int:comunicado_id>", type="http", auth="public", methods=["PUT", "OPTIONS"], csrf=False)
