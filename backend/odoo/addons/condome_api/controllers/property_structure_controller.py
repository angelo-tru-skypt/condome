...
class PropertyStructureController(http.Controller):
    ...
    @http.route("/condome_api/condominios/", type="http", auth="public", methods=["GET", "POST", "OPTIONS"], csrf=False, cors="http://localhost:3000")
    def condominiums(self, **kwargs):
        return service.handle_condominiums()

    @http.route("/condome_api/condominios/<int:condominio_id>", type="http", auth="public", methods=["GET", "PUT", "DELETE", "OPTIONS"], csrf=False, cors="http://localhost:3000")
    def condominium_detail(self, condominio_id, **kwargs):
        return service.handle_condominium_detail(condominio_id)

    @http.route("/condome_api/condominios/<int:condominio_id>/edificios/", type="http", auth="public", methods=["GET", "POST", "OPTIONS"], csrf=False, cors="http://localhost:3000")
    def buildings(self, condominio_id, **kwargs):
        return service.handle_buildings(condominio_id)

    @http.route("/condome_api/condominios/<int:condominio_id>/apartamentos/", type="http", auth="public", methods=["GET", "POST", "OPTIONS"], csrf=False, cors="http://localhost:3000")
