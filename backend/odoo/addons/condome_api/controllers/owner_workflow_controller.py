...
class OwnerWorkflowController(http.Controller):
    ...
    @http.route("/condome_api/owner/visitas/", type="http", auth="public", methods=["GET", "OPTIONS"], csrf=False, cors="*")
    def visits(self, **kwargs):
        return service.handle_visits()

    @http.route("/condome_api/owner/visitas/<int:visita_id>/decision", type="http", auth="public", methods=["PUT", "OPTIONS"], csrf=False, cors="*")
    def visit_decision(self, visita_id, **kwargs):
        return service.handle_visit_decision(visita_id)

    @http.route("/condome_api/owner/incidencias/", type="http", auth="public", methods=["GET", "OPTIONS"], csrf=False, cors="*")
    def incidents(self, **kwargs):
        return service.handle_incidents()

    @http.route("/condome_api/owner/incidencias/<int:incidencia_id>", type="http", auth="public", methods=["PUT", "OPTIONS"], csrf=False, cors="*")
    def incident_update(self, incidencia_id, **kwargs):
        return service.handle_incident_update(incidencia_id)

    @http.route("/condome_api/owner/notificaciones/<int:notification_id>/read", type="http", auth="public", methods=["PUT", "OPTIONS"], csrf=False, cors="*")
