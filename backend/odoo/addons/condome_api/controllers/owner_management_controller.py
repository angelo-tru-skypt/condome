...
class OwnerManagementController(http.Controller):
    ...
    @http.route("/condome_api/owner/<path:anything>", type="http", auth="public", methods=["OPTIONS"], csrf=False)
    def options(self, **kwargs):
        return service.build_response({"ok": True})

    @http.route("/condome_api/owner/propietarios/", type="http", auth="public", methods=["GET", "POST", "OPTIONS"], csrf=False)
    def owners(self, **kwargs):
        return service.handle_owners()

    @http.route("/condome_api/owner/propietarios/<int:propietario_id>", type="http", auth="public", methods=["PUT", "OPTIONS"], csrf=False)
