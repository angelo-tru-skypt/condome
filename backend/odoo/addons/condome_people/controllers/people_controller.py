...
class CondomePeopleController(http.Controller):
    ...
    @http.route("/condome_api/resident/context/", type="http", auth="public", methods=["GET", "OPTIONS"], csrf=False)
    def resident_context(self, **kwargs):
        return service.handle_resident_context()

    @http.route("/condome_api/condominios/<int:condominio_id>/residentes/", type="http", auth="public", methods=["GET", "OPTIONS"], csrf=False)
    def residents(self, condominio_id, **kwargs):
        return service.handle_residents(condominio_id)

    @http.route("/condome_api/apartamentos/<int:apartamento_id>/residentes/", type="http", auth="public", methods=["POST", "OPTIONS"], csrf=False)
    ...
    @http.route("/condome_api/residentes/<int:resident_id>/resend_credentials", type="http", auth="public", methods=["POST", "OPTIONS"], csrf=False)
    def resend_resident_credentials(self, resident_id, **kwargs):
        return service.handle_resend_resident_credentials(resident_id)

    @http.route("/condome_api/propietarios/<int:propietario_id>/resend_credentials", type="http", auth="public", methods=["POST", "OPTIONS"], csrf=False)
