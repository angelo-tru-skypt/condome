...
class OwnerReportingController(http.Controller):
    ...
    @http.route("/condome_api/owner/reportes/", type="http", auth="public", methods=["GET", "PUT", "OPTIONS"], csrf=False)
    def reports_summary(self, **kwargs):
        return service.handle_reports_summary()

    @http.route("/condome_api/owner/reportes/exportaciones/", type="http", auth="public", methods=["GET", "POST", "OPTIONS"], csrf=False)
    def report_exports(self, **kwargs):
        return service.handle_export_requests()

    @http.route("/condome_api/owner/reportes/exportaciones/<int:export_id>", type="http", auth="public", methods=["DELETE", "OPTIONS"], csrf=False)
    def report_exports_delete(self, export_id, **kwargs):
        return service.handle_export_delete(export_id)

    @http.route("/condome_api/report/download/<int:export_id>", type="http", auth="public", methods=["GET", "OPTIONS"], csrf=False)
